...
int main(int argc, char*argv[]) {
	...
	{ // This scope is for the purpose of removing false memory leak positives
		...
		if (timing_mode)
		{
			...
			// CHANGE HERE TO TRY DIFFERENT IMPLMENTATIONS
			...
			/*
			Ped::IMPLEMENTATION implementation_to_test_snd = Ped::OMP;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
				model.setup(parser.getAgents(), parser.getWaypoints(), implementation_to_test_snd);
				PedSimulation simulation(model, mainwindow);
				// Simulation mode to use when profiling (without any GUI)
				std::cout << "Running target version...\n";
				auto start = std::chrono::steady_clock::now();
			...
			Ped::IMPLEMENTATION implementation_to_test_thrd = Ped::PTHREAD;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
				model.setup(parser.getAgents(), parser.getWaypoints(), implementation_to_test_thrd);
				PedSimulation simulation(model, mainwindow);
				// Simulation mode to use when profiling (without any GUI)
				std::cout << "Running target version...\n";
				auto start = std::chrono::steady_clock::now();
